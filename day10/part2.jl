...
const scores = Dict(')' => 1, ']' => 2, '}' => 3, '>' => 4)
const match = Dict('(' => ')', '[' => ']', '{' => '}', '<' => '>')
stack = Array{Char}(undef, 100)
cnts = map(eachline("input.txt")) do l
  p = 1
  for c in l
    if haskey(match, c)
      stack[p] = match[c]
      p += 1
    elseif p > 1 && stack[p-1] == c
      p -= 1
    else
      return 0
    end
  end
  return foldr((i,t) -> t * 5 + scores[stack[i]], 1:(p-1); init=0)
end

# find the middle score
sort!(cnts)
idx = findfirst(>(0), cnts)
idx += (length(cnts) - idx) >>> 1
println("Result: ", cnts[idx])
