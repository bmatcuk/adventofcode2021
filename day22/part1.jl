...
const (minz,maxz) = (-50,50)

# build grid - read input
grid = falses(maxz-minz+1, maxy-miny+1, maxx-minx+1)
foreach(eachline("input.txt")) do l
  ...
  # ensure the ranges cross inside the initialization area
  all([
    minx <= x1 <= maxx || minx <= x2 <= maxx,
    miny <= y1 <= maxy || miny <= y2 <= maxy,
    minz <= z1 <= maxz || minz <= z2 <= maxz
  ]) || return

  # clamp and translate
  x1 = clamp(x1, minx, maxx) - minx + 1
  x2 = clamp(x2, minx, maxx) - minx + 1
  y1 = clamp(y1, miny, maxy) - miny + 1
  y2 = clamp(y2, miny, maxy) - miny + 1
  z1 = clamp(z1, minz, maxz) - minz + 1
  z2 = clamp(z2, minz, maxz) - minz + 1

  # update
  grid[z1:z2, y1:y2, x1:x2] .= instr == "on"
end

println("Result: ", count(grid))
