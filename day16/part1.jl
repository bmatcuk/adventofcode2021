...
total_versions = 0
while !iseob()
  global total_versions += read_bits(3)
  id = read_bits(3)
  if id == 4
    # literal value
    v = read_bits(5)
    ...
    lentype = read_bits(1)
    if lentype == 0
      # Next 15 bits are the number of bits that make up sub packets, but we
      # don't really care about that at the moment... just consume the 15 bits
      read_bits(15)
    else
      # Next 11 bits are the number of sub packets. But we don't really care
      # about that at the moment, so, just consume the 11 bits
      read_bits(11)
    end
  end
end

println("Result: ", total_versions)
