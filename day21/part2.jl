...
# read input
positions = map(l -> Int8(l[end] - '0'), eachline("input.txt"))

# map from total of three rolls to the number of ways to get that total
rolls = Dict{Int8,Int}()
for t in [Int8(x+y+z) for x in 1:3, y in 1:3, z in 1:3]
  rolls[t] = get(rolls, t, 0) + 1
end

# map from a state (player positions and scores) to the number of ways to get
# to that state
states = Dict((positions...,0,0) => 1)

# For each state, calculate the next states by playing all possible moves for
# the current player's turn.
wins = zeros(Int, length(positions))
turn = 0
while length(states) > 0
  nextstates = Dict{keytype(states),valtype(states)}()
  for (state,n) in states
    for (roll, nrolls) in rolls
      (p1,p2,s1,s2) = state
      nextn = n * nrolls
      if (turn & 1) == 0
        p1 = (p1 + roll - 1) % 10 + 1
      ...
      k = (p1,p2,s1,s2)
      nextstates[k] = get(nextstates, k, 0) + nextn
    end
  end
  global states = nextstates
  global turn = xor(turn, 1)
end

println("Result: ", maximum(wins))
