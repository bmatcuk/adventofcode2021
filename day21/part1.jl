...
# read input
positions = map(l -> l[end] - '0', eachline("input.txt"))

# do eet
rolls = Iterators.Stateful(Iterators.cycle(1:100))
scores = zeros(Int, length(positions))
turn = 0
while all(s -> s < 1000, scores)
  player = (turn & 1) + 1
  position = positions[player] = (positions[player] + sum(Iterators.take(rolls, 3)) - 1) % 10 + 1
  scores[player] += position
  global turn += 1
end

println("Result: ", scores[scores .< 1000][1] * turn * 3)
